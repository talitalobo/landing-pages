...
```{r,cache=FALSE, warning=FALSE, fig.height= 5, fig.width=9}  
ggplot(economics, aes(x=date, y=unemploy)) + geom_line() + xlab("Ano") + ylab("Desemprego (em milhares)") + ggtitle("Economics") # + geom_smooth() # + geom_smooth(method=lm)
```

* Gráfico de barra

```{r,cache=FALSE, warning=FALSE, fig.height= 5, fig.width=9}  
ggplot(iris, aes(factor(pop))) + geom_bar()
```
